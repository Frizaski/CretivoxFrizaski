...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLDivElement) {
    ...
    // Initialize to center coordinates to prevent jumping to (0,0)
    const rect = container.getBoundingClientRect();
    const startX = rect.width > 0 ? rect.width / 2 : window.innerWidth / 2;
    const startY = rect.height > 0 ? rect.height / 2 : 250;

    this.mousePos = { x: startX, y: startY };
    this.lastMousePos = { x: startX, y: startY };
    this.cacheMousePos = { x: startX, y: startY };
...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const vNum = typeof variant === 'string' ? parseInt(variant, 10) : variant;
    const Cls = variantMap[vNum] || variantMap[1];
